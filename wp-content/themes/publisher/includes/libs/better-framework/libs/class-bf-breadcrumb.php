...
class BF_Breadcrumb {
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'publisher' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'publisher' ),
			'home'                => esc_html__( 'Home', 'publisher' ),
			'error_404'           => esc_html__( '404 Not Found', 'publisher' ),
			'archives'            => esc_html__( 'Archives', 'publisher' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'publisher' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'publisher' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'publisher' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'publisher' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = array(
				'link' => home_url( $_SERVER['REQUEST_URI'] ),
				'name' => sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'publisher' ) ) ),
				'type' => 'current',
			);
	...
	protected function add_minute_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = array(
				'link' => home_url( $_SERVER['REQUEST_URI'] ),
				'name' => sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'publisher' ) ) ),
				'type' => 'current',
			);
	...
	protected function add_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = array(
				'link' => home_url( $_SERVER['REQUEST_URI'] ),
				'name' => sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'publisher' ) ) ),
				'type' => 'current',
			);
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'publisher' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'publisher' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'publisher' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'publisher' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'publisher' ) ) );
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					...
					$this->items[] = array(
						...
						'name' => sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'publisher' ) ) ),
					);
				...
				elseif ( '%monthnum%' == $tag ) {
					...
					$this->items[] = array(
						...
						'name' => sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'publisher' ) ) ),
					);
				...
				elseif ( '%day%' == $tag ) {
					...
					$this->items[] = array(
						...
						'name' => sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'publisher' ) ) ),
					);
	...
	function bbp_user() {
		...
		if ( bbp_is_replies_created() ) {
			$current = __( 'Replies', 'publisher' );
		} elseif ( bbp_is_topics_created() || bbp_is_single_view() ) {
			$current = __( 'Topics', 'publisher' );
		} elseif ( bbp_is_favorites() ) {
			$current = __( 'Favorites', 'publisher' );
		} elseif ( bbp_is_subscriptions() ) {
			$current = __( 'Subscriptions', 'publisher' );
		} elseif ( bbp_is_single_user_edit() ) {
			$current = __( 'Profile edit', 'publisher' );
		} elseif ( bbp_is_single_user() ) {
			$current = __( 'Profile', 'publisher' );
		}
