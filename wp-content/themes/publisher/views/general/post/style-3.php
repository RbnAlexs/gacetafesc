...
if ( publisher_get_header_style() !== 'disable' ) {
	...
	if ( publisher_show_breadcrumb() ) {
		Better_Framework()->breadcrumb()->generate( array(
			...
			'after'        => '</div>',
			'custom_class' => 'bc-top-style' . ( empty( $img['src'] ) ? ' bc-hide-last' : '' )
		) );
...
<div class="content-wrap">
	...
	<main <?php publisher_attr( 'content', '' ); ?>>
		...
		<div class="container <?php echo $layout_setting['container']; // escaped before ?> post-template-3">
			<?php
			...
			?>
			<div
					class="post-header post-tp-3-header <?php echo ! empty( $img['src'] ) ? 'bs-lazy wfi' : 'nfi'; ?>" <?php ! empty( $img['src'] ) ? publisher_the_thumbnail_attr( $thumbnail_size ) : ''; ?>>
