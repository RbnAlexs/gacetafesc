...
if ( ! $post_settings['meta']['show'] ) {
	$container_class .= ' post-template-6-no-meta';
}

if ( ! $post_settings['term'] ) {
	$container_class .= ' post-template-6-no-term';
}
...
<div class="content-wrap">
	...
	<main <?php publisher_attr( 'content', '' ); ?>>
		<div
				class="container <?php echo $layout_setting['container']; // escaped before ?> post-template-6 <?php echo ! empty( $img['src'] ) ? 'bs-lazy wfi' : 'nfi'; ?>">
			<?php
			...
			if ( ! empty( $img['src'] ) ) {
				...
				?>
				<div
						class="post-header post-tp-6-header bs-lazy wfi" <?php publisher_the_thumbnail_attr( $thumbnail_size ); ?>>
					<?php
					...
					if ( ! empty( $credit ) ) {
						?>
						<span class="image-credit"><?php echo $credit; // escaped before ?></span>
						<?php
					}
						...
						<div class="<?php echo $column['class']; ?>">
							<div class="single-container">
								<article <?php publisher_attr( 'post', 'single-post-content' ); ?>>
									<div
											class="post-header-inner <?php echo ! empty( $img['src'] ) ? 'wfi' : 'nfi'; ?>">
